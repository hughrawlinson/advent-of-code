...
const Cavern = {
  ...
  all({ contents }: Cavern, predicate: (d: Dumbo) => boolean): boolean {
    return contents.reduce<boolean>(
      (acc, line) =>
        acc &&
        line.reduce<boolean>(
...
if (require.main === module) {
  ...
  let counter = 0;
  while (!Cavern.all(cavern, (dumbo) => dumbo.energyLevel === 0)) {
    cavern = Cavern.step(cavern);
    counter++;
  }
  console.log(counter);
}
