...
export const LightGrid = {
  ...
  turn(lg: LightGrid, instruction: TurnInstruction): LightGrid {
    ...
    for (let y = start.y; y <= end.y; y++) {
      for (let x = start.x; x <= end.x; x++) {
        clone[y][x] =
          instruction.onOrOff === "on"
            ? clone[y][x] + 1
            : Math.max(0, clone[y][x] - 1);
      }
    }
    return clone;
  },
  ...
  totalBrightness(lg: LightGrid): number {
    return lg.reduce(
      (acc, row) => acc + row.reduce((acc, light) => acc + light, 0),
      0
    );
  },
};
...
if (require.main === module) {
  ...
  console.log(LightGrid.totalBrightness(resultingGrid));
}
